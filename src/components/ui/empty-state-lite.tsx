    ...
    return (
        <div className="flex flex-col h-full items-center text-center px-4 animate-fade-in overflow-hidden relative">
            ...
            <div className="flex-1 flex flex-col items-center justify-center w-full relative -mt-4">
                {/* 
                   Desktop Ring Replication (Gyroscope Effect) - MAXIMIZED SCALE
                   - Scaled up to fill width (350px)
                   - Brighter Opacity (50%)
                */}
                <div className="relative w-[360px] h-[360px] flex items-center justify-center mb-8 pointer-events-none">
                    {/* Center Glow */}
                    ...
                    <div className="absolute w-[340px] h-[170px] rounded-[100%] border border-cyan-400/50 border-dashed animate-spin-slow shadow-[0_0_50px_rgba(34,211,238,0.2)]" />

                    {/* Ring 2 - Verticalish (Counter-Clockwise) - Violet Accent */}
                    <div className="absolute w-[170px] h-[340px] rounded-[100%] border border-violet-400/50 border-dashed animate-spin-reverse-slow shadow-[0_0_50px_rgba(139,92,246,0.2)]" />
                </div>

                {/* 3. MIDDLE: Suggestions (2x2 Grid Stack) - FIXED DIMENSIONS & BIGGER */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 mt-2 w-full max-w-[360px] grid grid-cols-2 gap-3 px-3 animate-fade-in-up animation-delay-300 z-20">
                    {suggestions.map((s, i) => (
                        <div
                            key={i}
                        ...
                        >
                            <SpotlightCard
                                className="h-[90px] p-4 flex flex-row items-center gap-3 rounded-2xl border border-indigo-500/40 bg-black/80 backdrop-blur-xl shadow-lg shadow-black/50 hover:bg-white/5 transition-colors"
                            >
                                <div className="p-2.5 rounded-lg bg-indigo-500/20 text-indigo-300 w-fit shrink-0">
                                    <s.icon size={20} />
                                </div>
                                <span className="text-sm font-medium text-indigo-100/90 leading-tight line-clamp-2 text-left">{s.text}</span>
                            </SpotlightCard>
                        </div>
                    ))}
                </div>
